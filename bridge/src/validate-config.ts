import { bridgeRuntimeMode, createBridgeRuntime, validateBridgeApiUrl } from './index.js';
...
export function validateBridgeRehearsalConfiguration(environment: RuntimeEnvironment = process.env) {
  ...
  const bridgeApi = validateBridgeApiUrl(value(environment, 'BRIDGE_API_URL'), mode);
