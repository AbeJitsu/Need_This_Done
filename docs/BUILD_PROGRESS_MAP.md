...
**Branch:** `refactor/workflow-planner-naming-2026-09-19`
...
| Local control plane | Disposable local Supabase has migration 113/RLS proof, and MCP creates and reads an approval-gated workflow | Next proof | `npm run verify:database` plus `npm run test:workflow-mcp:local` with the real local Supabase gate passing |
...
default MCP dispatcher still fails closed because the durable workflow adapter is
