...
**Operator-only private boundary (2026-08-23):** Migration `105` preserves
historical client-linked users, project links, memberships, files, comments,
handoffs, and delivery records while removing their ability to authorize
private access. Private route handlers and `/dashboard` and `/employee` now
require the admin/operator role; unauthenticated API requests return `401` and
...
Disposable local Supabase reset cleanly through `105`. `verify:database`
passed schema lint plus 9 schema, 14 security, 10 AI-employee RLS, 3 agent
operations, 2 planner, 2 prospecting, and 1 consultation checks. The staged
migration verifier passed 33 mappings and 17 gates. Unit tests passed 240/240;
...
customer-data handling, or hosted database state changed. The historical
item-8 change added only `ENV_TARGET`; a separate owner-directed 2026-08-18
configuration added the private `OPENROUTER_BACKUP_MODEL` to Production and
Preview. A later, separate owner directive deployed the reviewed application
slice to Production without making a provider request or changing hosted
Supabase.
...
**OpenRouter boundary:** The API key and exact model IDs remain private environment variables. Step 10A recorded exactly six sanitized comparison requests and provider usage in hosted Supabase; DeepSeek completed all three tasks, while Nemotron was rejected by the provider privacy/data-policy guardrail before generation. A fresh 2026-08-18 public catalog check confirmed no `deepseek/deepseek-v4-flash:free` entry and confirmed zero-priced `google/gemma-4-26b-a4b-it:free`. The hosted comparison profile remains `evaluation-required` with no selected model. The root `.env.local` points to `.env.local.profile`; `app/.env.local` points to the root active profile. The exact Gemma backup is now configured privately in the local profile and Vercel Production/Preview. The server rejects missing, moving, or dynamic-router primary IDs; migrations `096`–`097` persist actual probe endpoints and retire the hardcoded DeepSeek-free route. Production deployment `dpl_7kr6p3LBfph9VjLMBnYgV627BE2M` is `READY` at `https://needthisdone.com`; the probe still cannot pin a route, and no new live provider request occurred. Evidence: [Step 10A model comparison](launch/step-10a-model-comparison-2026-08-17.md).
...
- On 2026-08-22, `npm audit --omit=dev --audit-level=high` found **0 vulnerabilities**. The prior dependency-audit blocker is removed; rerun this command before any future promotion. Owner: NeedThisDone engineering owner.
...
- The globally installed Vercel CLI `37.2.1` is below the current deployment-endpoint minimum and was rejected before deployment. An ephemeral `vercel@latest` (`59.1.4`) completed the deployment but its npm install emitted deprecated-package notices, including a `tar@7.5.7` security-maintenance warning. Owner: engineering. Upgrade and audit the pinned deployment tooling before the next deployment; review/removal date: 2026-09-15.
- Item 8 is an approved retention exception recorded in [the environment inventory](launch/ENVIRONMENT_VARIABLE_INVENTORY.md). The historical preflight found 55 names per scope and added only `ENV_TARGET` to Production/Preview. The 2026-08-18 owner directive added only private `OPENROUTER_BACKUP_MODEL` to those same scopes; the current names-only count is 64/64/55 for Production/Preview/Development. Existing unrelated variables were not revoked or rotated. Owner: Abe Reyes / `abejitsu`; review/removal date: 2026-09-15. The variable change was not provider activation; a later explicit directive separately approved the Production deployment. Items 10–22 remain blocked.
...
- Step 10A model comparison ran on 2026-08-17 under the explicitly scoped six-request approval. The live catalog contained both exact IDs; hosted Supabase now retains six evaluation rows and six reconciled usage rows totaling `$0.000122624`. DeepSeek returned valid structured JSON for all three tasks; Nemotron’s three requests were rejected by the OpenRouter privacy/data-policy guardrail and were recorded as failed/repair-required. The profile remains `evaluation-required`, with no model pin, schedule, task, prospect, outreach, or external recipient. The sanitized evidence is [step-10a-model-comparison-2026-08-17.md](launch/step-10a-model-comparison-2026-08-17.md). A privacy-policy change, repeat comparison, and model pin require separate approval; items 10–22 remain blocked.
- On 2026-08-18, a read-only account-filtered OpenRouter catalog check identified `google/gemma-4-31b-it:free` as the recommended next free comparison candidate: text output, zero prompt/completion pricing, 262,144-token context, and current `response_format`/tools support. `google/gemma-4-26b-a4b-it:free` remains the multi-endpoint backup. No provider completion, Vercel variable change, hosted write, or model pin occurred; repeat comparison remains separately approval-gated.
- Migration `098_fail_closed_openclaw_schedule_and_provenance.sql` is applied only to disposable local Supabase. It makes run timing/emergency-stop database-authoritative, combines exact Gateway model provenance, usage reconciliation, and approved-task completion in one service-role transaction, and retires dashboard run creation in favor of frozen-plan dispatch. Local reset applied `001`–`098`; TypeScript, planner RLS 2/2, schema manifest 9/9, and bridge offline tests 8/8 passed on 2026-08-19. Hosted history remains `91/095`; a hosted migration, provider probe, Mac installation, or activation still requires its own approval. Owner: Abe Reyes / private-operator owner. Review/removal date: 2026-09-15 or before any backup activation.
...
**Operational boundaries:** Production deployment `dpl_7kr6p3LBfph9VjLMBnYgV627BE2M` is `READY` and aliased to `https://needthisdone.com`; the previously reviewed Preview deployment remains unchanged. The hosted queue read remains compatible with hosted schema `095`, while the signed backup probe stays unavailable until migrations `096`–`097` receive separate hosted approval. No existing credential was revoked or rotated and no provider request was made. Payments remain on the `/contact` fallback; real sender delivery, bridge execution, model pinning, publication, spend, account changes, and external messages remain separately gated. The comparison-only provider calls are documented in Step 10A above.

**Rollback:** Hosted `090`–`095` completed successfully; all database security repairs are forward-only. The immediate application rollback is the prior Production deployment `dpl_4XP38V8P6G8NGBb517aMa658m5Qm`; `8b8d429` remains the historical code rollback reference. Preserve hosted migration history, evidence, backups, and media records rather than deleting them. Never recreate anonymous Storage policies, reset hosted Supabase, or reverse migrations ad hoc.
...
| 2026-08-17 | Step 10A OpenRouter model comparison | This working slice | Hardened live catalog parsing for provider extensions, omitted structured-output request parameters for models that do not advertise them, recorded catalog metadata, and locked comparison writes to `evaluation-required`. The hosted run made exactly six sanitized completion requests: DeepSeek completed 3/3 with `$0.000122624` total provider cost; Nemotron was rejected 3/3 by the provider privacy/data-policy guardrail. Six evaluation and six reconciled usage records were retained; the route stayed unselected and no worker/live external action ran. | Preserve model-evaluation and usage evidence. Keep `evaluation-required`; do not rerun after a provider-policy change or pin a model without separate approval. |
