...
| `/system` | Repeated flows made the architecture harder to follow, and the first draft implied ChatGPT and named machines were required dependencies. | “NeedThisDone helps teams and individuals solve technology problems and simplify repeated work with clear, focused solutions.” The page previews its structure, shows one six-step plain-English operating path, explains why each technical layer matters, and ends with a direct comparison between ordinary LLM chat and NeedThisDone. It names ChatGPT, Claude, and any compatible LLM as clients, and local or cloud computers as worker hosts. Proof-state labels distinguish the design from live connections. | Architecture note | Implemented; owner review pending |
