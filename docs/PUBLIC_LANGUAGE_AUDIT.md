...
| `/system` | Repeated flows made the architecture harder to follow, and the first draft implied ChatGPT was a required dependency. | “NeedThisDone helps teams and individuals solve technology problems and simplify repeated work with clear, focused solutions.” One six-step plain-English operating path comes before the named technical stack; it explains that any compatible LLM can call the same MCP/API contract and that ChatGPT is only the current client. Proof-state labels distinguish the design from live connections. | 22 | Implemented; owner review pending |
