...
| Retained public and workspace routes work on desktop and mobile. | Playwright smoke tests with no console errors, overflow, accessibility violations, or unexplained skips. | Sanitized report fixture enabled all 8 core smoke checks; all 4 employee workspace checks pass on desktop and mobile. | Proven locally; authenticated authorization proof remains separate. |
...
| The code is safe to promote. | Lint, TypeScript, unit tests, accessibility tests, production build, retirement guards, database gate when applicable, and `git diff --check`. | The deterministic code gate passes with zero skips. After a fresh reset through `078`, the database gate passes 30/30 checks including the exact retained-schema manifest and financial isolation. | Local gates proven; hosted behavior and deployment approval remain separate. |
