...
if (pendingVersions[0] !== '096' || pendingVersions.at(-1) !== '109') {
  fail('pending stages must cover exactly 096–109');
}
...
for (const [label, seen] of [['new', seenNewVersions], ['original', seenOriginalVersions]]) {
  if (seen.size !== expectedVersions.length || expectedVersions.some((version) => !seen.has(version))) {
    fail(`${label} migration versions are not an exact one-to-one map for 073–109`);
  }
}
...
if (stagedVersions.length !== expectedVersions.length || expectedVersions.some((version) => !stagedVersions.includes(version))) {
  fail('stage coverage is not an exact one-to-one map for 073–109');
}
...
if (JSON.stringify(pendingStageVersions) !== JSON.stringify(pendingVersions)) fail('pending stages are not contiguous 096–109');
