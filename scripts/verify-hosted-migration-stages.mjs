...
if (pendingVersions[0] !== '096' || pendingVersions.at(-1) !== '115') {
  fail('pending stages must span 096–115');
}
...
if (JSON.stringify(pendingStageVersions) !== JSON.stringify(pendingVersions.filter((version) => !retiredLocalOnlyVersions.includes(version)))) fail('pending stages do not cover exactly 096–106 and 110–115');

console.log(`Hosted migration staging verified: ${migrations.length} mappings, ${stages.length} gates; historical map through ${manifest.expected_hosted_latest}; current promotion baseline ${manifest.current_hosted_promotion_baseline}; pending 096–106 and 110–115 (107–109 intentionally retired local-only). SQL hashes preserved.`);
for (const stage of stages) {
