...
DECLARE
  ...
  v_usd_amount DECIMAL(20, 10);
  v_result DECIMAL(20, 10);
BEGIN
  ...
  v_result := v_usd_amount * v_to_rate;

  RETURN ROUND(v_result)::INTEGER;
END;
$$;

-- format_price (from 027)
...
DECLARE
  v_currency RECORD;
  v_formatted TEXT;
  v_decimal_amount DECIMAL(20, 2);
BEGIN
  SELECT * INTO v_currency FROM currencies WHERE code = p_currency;

  IF v_currency IS NULL THEN
    RETURN '$' || (p_amount / 100.0)::TEXT;
  END IF;

  v_decimal_amount := p_amount / POWER(10, v_currency.decimal_digits);

  v_formatted := TO_CHAR(v_decimal_amount, 'FM999,999,999.' || REPEAT('0', v_currency.decimal_digits));
...
BEGIN
  ...
  IF v_template.price_type = 'paid' THEN
    IF p_user_id IS NULL THEN
      RETURN json_build_object('success', false, 'error', 'Login required for paid templates');
    END IF;
    ...
    IF NOT v_already_purchased THEN
      RETURN json_build_object('success', false, 'error', 'Purchase required');
    END IF;
  END IF;
  ...
  RETURN json_build_object(
    ...
    'template', json_build_object(
      ...
      'puck_data', v_template.puck_data
    )
  );
END;
$$;
