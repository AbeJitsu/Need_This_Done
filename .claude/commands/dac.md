...
3. **CRITICAL: NO Claude Code signature - EVER**
...
Once the user approves (says "no" to adjustments, "looks good", "ship it", etc.), provide a **single command** that commits AND pushes in one go:

```bash
git commit -m "Your commit message here" && git push origin main
```

This follows the terminal preference for chaining commands with `&&`.
