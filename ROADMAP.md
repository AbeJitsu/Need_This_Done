...
| 5. Retire legacy systems | **Current — independent slices only** | Remove Medusa/Railway, LMS, editor, old commerce, and obsolete APIs, jobs, tests, providers, and docs. | Phase 3 for disconnected systems; Phase 4 plus caller-removal evidence for commerce and order-linked systems. | Each retired subsystem is absent from builds, routes, jobs, and supported documentation. |
...
Phase 4's repository catalog, guarded hosted-link handoff, and test-link tooling remain in place, but Stripe test creation and end-to-end validation are unfinished and owner-deferred. Phase 5 may proceed only with independently disconnected systems such as the LMS and editor. Commerce, order-linked appointments, and payment data cannot be retired until Phase 4 validation and their explicit caller-removal evidence are complete.
