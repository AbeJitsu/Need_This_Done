...
Phase 6 local validation uses the Supabase CLI against a disposable local database.
As of 2026-07-29 the CLI is installed, but this workstation has no Docker-compatible
container engine, so migration reset and behavioral RLS execution are still blocked.
Hosted Supabase must not be used as a substitute test environment. After the local
gate passes, migration `072` receives a hosted dry run and separate deployment approval.
