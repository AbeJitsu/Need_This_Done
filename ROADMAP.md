...
| Installed agent foundation | Hermes; OpenClaw; Codex; OpenRouter boundary; Supabase pgvector later | Hermes and OpenClaw CLIs are installed locally. Hermes has a separate ChatGPT/Codex OAuth session and passed a harmless read-only Codex-runtime proof. No agent gateway, daemon, production adapter, or OpenRouter key is authorized. OpenRouter setup begins with a hard cost limit and model allowlist. Supabase remains durable workflow state. |
