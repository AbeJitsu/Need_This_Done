...
const retainedTables = [
  ...
  'daily_desk_cost_reservations',
  'daily_desk_decisions',
  'daily_desk_follow_ups',
  'daily_desk_operating_briefs',
  'daily_desk_outcomes',
  'daily_desk_prospect_cards',
  'daily_desk_runs',
  'daily_desk_social_asset_versions',
  'daily_desk_social_assets',
...
const requiredPolicies = [
  ...
  ['daily_desk_cost_reservations', 'owners read daily desk cost reservations', 'SELECT'],
  ['daily_desk_decisions', 'owners read daily desk decisions', 'SELECT'],
  ['daily_desk_follow_ups', 'owners read daily desk follow ups', 'SELECT'],
  ['daily_desk_operating_briefs', 'owners read daily desk briefs', 'SELECT'],
  ['daily_desk_outcomes', 'owners read daily desk outcomes', 'SELECT'],
  ['daily_desk_prospect_cards', 'owners read daily desk prospect cards', 'SELECT'],
  ['daily_desk_runs', 'owners read daily desk runs', 'SELECT'],
  ['daily_desk_social_asset_versions', 'owners read daily desk social versions', 'SELECT'],
  ['daily_desk_social_assets', 'owners read daily desk social assets', 'SELECT'],
...
localDescribe.sequential('retained Supabase schema manifest', () => {
  ...
  it('preserves the critical retained columns and types', async () => {
    const expected = [
      ...
      ['daily_desk_cost_reservations', 'reserved_cost', 'numeric'],
      ['daily_desk_decisions', 'idempotency_key', 'uuid'],
      ['daily_desk_follow_ups', 'state', 'text'],
      ['daily_desk_operating_briefs', 'revision', 'int4'],
      ['daily_desk_outcomes', 'outcome_type', 'text'],
      ['daily_desk_prospect_cards', 'citations', 'jsonb'],
      ['daily_desk_runs', 'local_date', 'date'],
      ['daily_desk_runs', 'route_estimated_cost', 'numeric'],
      ['daily_desk_social_asset_versions', 'storage_path', 'text'],
      ['daily_desk_social_assets', 'current_version_id', 'uuid'],
  ...
  it('preserves decision, queue, history, and cascading-cleanup constraints', async () => {
    const requiredConstraints = [
      ...
      'daily_desk_cost_reservations_reservation_key_key',
      'daily_desk_decisions_idempotency_key_key',
      'daily_desk_follow_ups_prospect_card_id_key',
      'daily_desk_operating_briefs_owner_id_revision_key',
      'daily_desk_outcomes_idempotency_key_key',
      'daily_desk_prospect_cards_run_id_deduplication_key_key',
      'daily_desk_runs_owner_id_local_date_key',
      'daily_desk_social_asset_versions_asset_id_version_number_key',
      'daily_desk_social_assets_run_id_key',
  ...
  it('keeps the retained trigger set and no retired public views', async () => {
    ...
    expect(triggers.rows.map((row) => row.trigger_name)).toEqual([
      ...
      'daily_desk_cost_reservations_updated_at',
      'daily_desk_follow_ups_updated_at',
      'daily_desk_runs_updated_at',
      'daily_desk_social_assets_updated_at',
      ...
      'prevent_daily_desk_decision_mutation',
      'prevent_daily_desk_social_version_mutation',
  ...
  it('keeps retained RPC signatures and execution grants narrow', async () => {
    const rpcChecks = [
      ...
      ['public.create_daily_desk_operating_brief(text,text,text,text,text)', true, true, false],
      ['public.prepare_daily_desk_run(uuid,date)', true, true, false],
      ['public.claim_daily_desk_run(uuid,text,integer)', true, false, false],
      ['public.claim_next_daily_desk_run(text,integer)', true, false, false],
      ['public.record_daily_desk_model_route(uuid,uuid,text,text,numeric,jsonb,text)', true, false, false],
      ['public.reserve_daily_desk_cost(uuid,uuid,text,uuid,text,numeric,numeric,jsonb,text)', true, false, false],
      ['public.reconcile_daily_desk_cost(uuid,numeric,jsonb)', true, false, false],
      ['public.complete_daily_desk_run(uuid,text,text,jsonb,text,uuid,text,jsonb)', true, false, false],
      ['public.record_daily_desk_follow_up_decision(uuid,text,text,text,date,text,uuid)', true, true, false],
      ['public.ensure_daily_desk_social_asset(uuid)', true, true, false],
      ['public.record_daily_desk_social_version(uuid,uuid,text,text,text,text,jsonb)', true, true, false],
      ['public.record_daily_desk_social_decision(uuid,text,text,uuid)', true, true, false],
      ['public.record_daily_desk_outcome(uuid,uuid,text,text,timestamp with time zone,uuid)', true, true, false],
    ] as const;
