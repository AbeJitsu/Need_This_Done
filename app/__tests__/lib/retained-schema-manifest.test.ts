...
const requiredPolicies = [
  ...
  ['operator_weekly_priorities', 'owners read weekly priorities', 'SELECT'],
    ...
    ['mcp_workflows', 'authenticated owners read MCP workflows', 'SELECT'],
...
localDescribe.sequential('retained Supabase schema manifest', () => {
  ...
  it('preserves the critical retained columns and types', async () => {
    const expected = [
      ...
      ['mcp_workflows', 'approval_required', 'bool'],
      ['mcp_workflows', 'idempotency_key', 'uuid'],
      ['mcp_workflows', 'request_hash', 'text'],
      ['mcp_workflows', 'status', 'text'],
  ...
  it('preserves decision, queue, history, and cascading-cleanup constraints', async () => {
    const requiredConstraints = [
      ...
      'mcp_workflows_approval_state_check',
      'mcp_workflows_request_check',
      'mcp_workflows_request_hash_check',
      'mcp_workflows_result_ref_check',
      'mcp_workflows_summary_check',
      'mcp_workflows_worker_check',
    ];
  ...
  it('keeps the retained trigger set and no retired public views', async () => {
    ...
    expect(triggers.rows.map((row) => row.trigger_name)).toEqual([
      ...
      'update_mcp_workflows_updated_at',
