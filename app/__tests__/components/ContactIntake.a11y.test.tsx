...
const { searchParams } = vi.hoisted(() => ({
  searchParams: new URLSearchParams('offer=website-fix'),
}));
...
describe('Contact intake accessibility', () => {
  it('labels the Website Fix intake and its selected radio option', async () => {
    const { container, getByRole } = render(<ContactPage />);
    expect(getByRole('radio', { name: /website fix/i })).toBeChecked();
  ...
  it('keeps the automation questions labeled after switching paths', async () => {
    ...
    await waitFor(() => expect(getByRole('radio', { name: /website fix/i })).toBeChecked());
    fireEvent.click(getByRole('radio', { name: /managed automation/i }));
