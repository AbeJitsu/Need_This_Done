...
test.describe('Checkout Flow Screenshots', () => {
  ...
  test('captures complete guest checkout flow with consultation product', async ({ page }) => {
    ...
    // ========================================================================
    // Step 5: Checkout - Appointment Step (for consultation products)
    // ========================================================================
    // Wait for appointment step to appear (consultation products require it)
    // The heading is "Schedule Your Consultation" from PageHeader
    await expect(appointmentHeading.or(paymentHeading)).toBeVisible({ timeout: 15000 });
    ...
    await expect(page.getByRole('heading', { name: /payment/i })).toBeVisible({ timeout: 20000 });

    // Wait for Stripe iframe to appear
    await expect(page.frameLocator('iframe[title*="Secure"]').nth(0).locator('body')).toBeVisible({ timeout: 20000 });

    // Give Stripe a moment to fully render
