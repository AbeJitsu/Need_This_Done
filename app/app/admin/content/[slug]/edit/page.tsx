...
import { iconCircleColors, containerBg, mutedTextColors, coloredLinkText, headingColors } from '@/lib/colors';
    ...
    return (
      <div className={`flex items-center justify-center min-h-screen ${containerBg.page}`}>
        <div className="text-center">
          ...
          <p className={mutedTextColors.normal}>Loading editor...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={`flex items-center justify-center min-h-screen ${containerBg.page}`}>
        <div className="text-center max-w-md">
          <div className={`w-12 h-12 ${iconCircleColors.red.bg} rounded-full flex items-center justify-center mx-auto mb-4`}>
            <svg className={`w-6 h-6 ${coloredLinkText.red}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
          </div>
          <h2 className={`text-lg font-semibold ${headingColors.primary} mb-2`}>
            Failed to Load
          </h2>
          <p className={`${mutedTextColors.normal} mb-4`}>{error}</p>
          <button
            onClick={() => router.push('/admin/content')}
            className={`px-4 py-2 text-sm font-medium ${coloredLinkText.blue} hover:underline`}
          >
