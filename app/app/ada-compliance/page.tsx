    ...
    <main className="bg-[#f7f4ed] text-[#183229]">
      <section className="relative overflow-hidden bg-[#18372e] text-white">
        ...
        <div className="relative mx-auto max-w-5xl px-5 py-20 sm:px-8 md:py-28">
          ...
          <p className="mt-6 max-w-3xl text-lg leading-8 text-emerald-50/75">The free audit surfaces selected accessibility, SEO, and performance signals. Website Fix can address one agreed issue; it does not promise legal compliance or replace specialist legal or accessibility advice.</p>
          <div className="mt-9 flex flex-col gap-3 sm:flex-row"><Link href="/site-analyzer" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-emerald-300 px-7 py-3 font-bold text-[#18372e]">Run the free site audit <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link><Link href="/contact?offer=website-fix" className="inline-flex min-h-12 items-center justify-center rounded-full border border-white/20 px-7 py-3 font-bold">Start a Website Fix</Link></div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-5xl px-5 py-16 sm:px-8 md:py-24"><div className="rounded-3xl bg-[#e4eee6] p-8 text-center"><ShieldCheck className="mx-auto h-7 w-7 text-[#126b4e]" aria-hidden="true" /><h2 className="mt-4 text-3xl font-black">Keep the promise narrower than the problem.</h2><p className="mx-auto mt-3 max-w-2xl leading-7 text-[#50675e]">The $500 Website Fix includes an evidence review and one agreed correction. Larger remediation programs, redesigns, and compliance certification are outside that contained scope.</p><Link href="/contact?offer=website-fix" className="mt-7 inline-flex min-h-12 items-center gap-2 rounded-full bg-[#126b4e] px-7 py-3 font-bold text-white">Start a Website Fix <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link></div></section>
    </main>
  );
}
