...
// ============================================================================
// Resume Page Client Component
// ============================================================================
// Animated version of the resume page using Framer Motion.
// Server component (page.tsx) handles metadata, this handles rendering.
...
const skills = [
  {
    ...
    items:
      'React, Next.js, Vue.js, TypeScript, Tailwind, Framer Motion, Liquid (Shopify), Visual Page Builders (Puck), WYSIWYG Editors (TipTap)',
  },
  {
    category: 'Backend & Data',
    items:
      'Node.js, Python, REST APIs, GraphQL, PostgreSQL, MongoDB, Supabase, Medusa, Redis, Schema Design, Zod',
  },
  {
  ...
  },
  {
    ...
    items:
      'LLM Integration (OpenAI, Claude), Prompt Engineering, RAG Systems, Chatbots, Vector Embeddings',
  },
  {
    category: 'Architecture',
    items:
      'Modular Components, Custom Hooks, Context Providers, Optimistic Updates, Cache-Aside Pattern, Turborepo, pnpm Workspaces',
  },
  {
...
export default function ResumePageClient() {
  return (
    <div className="max-w-4xl mx-auto px-8 sm:px-12 md:px-16 lg:px-20 py-8">
      ...
      <FadeIn triggerOnScroll={false} as="header">
        <div className={`text-center mb-6 pb-4 border-b-2 ${accentColors.blue.border}`}>
          <h1 className={`text-2xl md:text-3xl font-bold ${titleColors.blue} mb-1`}>
            ABIEZER &quot;ABE&quot; REYES
          </h1>
          <p
            className={`text-xs uppercase tracking-widest ${formInputColors.helper} mb-3`}
          >
            Full-Stack Developer
          </p>
          <div
            className={`flex flex-wrap justify-center gap-2 text-xs ${formInputColors.helper}`}
          >
      ...
      {/* Summary - animates on load with slight delay */}
      <FadeIn triggerOnScroll={false} delay={0.1}>
        <section className={`mb-6 p-4 rounded ${cardBgColors.elevated}`}>
          <div
            className={`text-sm leading-relaxed ${headingColors.secondary} space-y-3`}
          >
            <p>
              Over 25 years working with technology in business environments, shipping
              production software since 2023. I design systems that work, from AI
              automation to scalable architectures. The engineers I respect most never
              stop sharpening their tools, and neither do I.
            </p>
            <p>
              Before tech, I led a five-person medic team in the Army and earned three
              promotions at Toyota over seven years.
            </p>
            <p>
              What&apos;s stayed consistent: I stay calm when things break, I explain
              technical stuff so it makes sense the first time, and I follow through on
              what I say I&apos;ll do.
            </p>
          </div>
        </section>
      ...
      {/* Technical Skills - staggered grid */}
      <RevealSection>
        <section className={`mb-6 p-4 rounded ${cardBgColors.elevated}`}>
          <h2
          ...
          <StaggerContainer
            className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 ml-4"
      ...
      {/* Professional Experience - staggered entries */}
      <RevealSection>
        <section className={`mb-6 p-4 rounded ${cardBgColors.elevated}`}>
          <h2
            className={`text-sm font-bold uppercase tracking-wide ${titleColors.blue} mb-3 pb-2 border-b ${dividerColors.border}`}
          ...
          <StaggerContainer className="space-y-4 ml-4" triggerOnScroll={false}>
            {experience.map((job) => (
              <StaggerItem key={`${job.company}-${job.title}`}>
                ...
                <ul
                  className={`list-disc list-outside ml-6 space-y-1 text-xs ${headingColors.secondary}`}
                >
      ...
      <FadeIn>
        <section className={`mb-6 p-4 rounded ${cardBgColors.elevated}`}>
          ...
          <div className="space-y-3 ml-4">
            ...
            <ul
              className={`text-xs ${headingColors.secondary} list-disc list-outside ml-4 space-y-1`}
            >
              <li>Full-stack Next.js application serving real customers in production</li>
              <li>
                Designed PostgreSQL database schema for users, products, orders, and
                payments
              </li>
              ...
              <li>
                Ship reliably with Playwright E2E tests, Vitest unit tests, and CI/CD
              </li>
            </ul>
            <p className={`text-xs italic ${formInputColors.helper}`}>
              React/Next.js • TypeScript • Medusa (Headless Commerce) • PostgreSQL/Supabase
              • Redis • Stripe • Playwright • Vercel
            </p>
          </div>
        </section>
      </FadeIn>

      {/* Education - staggered entries */}
      <RevealSection>
        <section className={`mb-6 p-4 rounded ${cardBgColors.elevated}`}>
          <h2
            className={`text-sm font-bold uppercase tracking-wide ${titleColors.blue} mb-3 pb-2 border-b ${dividerColors.border}`}
          ...
          <StaggerContainer className="space-y-3 ml-4" triggerOnScroll={false}>
            {education.map((edu) => (
              <StaggerItem key={edu.institution}>
                <p className={`text-xs ${headingColors.secondary}`}>
                  <span className="font-semibold">{edu.institution}</span>{' '}
                  <span className={formInputColors.helper}>({edu.dates})</span>:{' '}
                  {edu.description}
                </p>
              </StaggerItem>
            ))}
      ...
      {/* Books That Shaped How I Work - staggered entries */}
      <RevealSection>
        <section className={`mb-6 p-4 rounded ${cardBgColors.elevated}`}>
          <h2
            className={`text-sm font-bold uppercase tracking-wide ${titleColors.blue} mb-3 pb-2 border-b ${dividerColors.border}`}
