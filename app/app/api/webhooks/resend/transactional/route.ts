...
export async function POST(request: Request) {
  ...
  if (!eventId || !verifyWebhookSecret(body, request.headers.get('x-resend-signature') || request.headers.get('svix-signature'), process.env.TRANSACTIONAL_RESEND_WEBHOOK_SECRET)) return NextResponse.json({ error: 'Invalid transactional webhook signature.' }, { status: 401 });
