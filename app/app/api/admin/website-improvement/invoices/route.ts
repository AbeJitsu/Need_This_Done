...
export async function POST(request: Request) {
  ...
  const operation = await admin.rpc('upsert_provider_operation', { target_provider: 'stripe', target_operation_type: 'website_improvement_start_invoice', target_idempotency_key: idempotencyKey, target_status: 'pending', target_request_metadata: { project_id: input.data.projectId, amount_cents: 25000, currency: 'usd', test_mode: true } });
  ...
  if (!adapter) {
    await admin.rpc('upsert_provider_operation', { target_provider: 'stripe', target_operation_type: 'website_improvement_start_invoice', target_idempotency_key: idempotencyKey, target_status: 'failed_retryable', target_error: 'Stripe test-mode adapter is disabled.' });
  ...
  try {
    ...
    await admin.rpc('upsert_provider_operation', { target_provider: 'stripe', target_operation_type: 'website_improvement_start_invoice', target_idempotency_key: idempotencyKey, target_status: 'succeeded', target_provider_reference: result.invoiceId });
    const reference = await admin.rpc('record_website_improvement_invoice_reference', { target_operation_id: operationId, target_project_id: input.data.projectId, target_stripe_invoice_id: result.invoiceId, target_status: 'open' });
    ...
    await admin.rpc('upsert_provider_operation', { target_provider: 'stripe', target_operation_type: 'website_improvement_start_invoice', target_idempotency_key: idempotencyKey, target_status: 'failed_retryable', target_error: error instanceof Error ? error.message : 'Stripe provider failed.' });
