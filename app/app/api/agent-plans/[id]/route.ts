...
function routeError(error: { code?: string; message?: string } | null, fallback: string) {
  if (migrationUnavailable(error)) return { error: 'Workflow plans are not configured yet.', status: 503 };
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const steps = (parsed.data.steps || current.steps) as WorkflowPlanStep[];
