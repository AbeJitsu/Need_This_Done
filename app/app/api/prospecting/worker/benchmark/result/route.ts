...
export async function POST(request: Request) {
  ...
  if (candidate.candidate_kind === 'deepseek-fallback') return NextResponse.json({ error: 'The retired hardcoded fallback cannot accept new results.' }, { status: 409 });
  ...
  if (comparisonCandidate) {
    ...
    if (!expectedModel) return NextResponse.json({ error: 'Configured model comparison is unavailable.' }, { status: 503 });
  ...
  if (parsed.data.actualModelId) {
    const { data: actualModel, error: actualModelError } = await admin.rpc('record_private_model_actual', {
      target_reservation_key: parsed.data.reservationKey,
      target_actual_model_id: parsed.data.actualModelId,
    });
    if (actualModelError || !actualModel) return NextResponse.json({ error: 'The actual routed model could not be recorded.' }, { status: 409 });
  }
  ...
  const { data: record, error: recordError } = await admin.from('model_evaluation_records').upsert({
    ...
    actual_model_id: parsed.data.actualModelId || null,
  ...
  const policy = selectModelRoutingPolicy(nextRecords, freeCandidates);
