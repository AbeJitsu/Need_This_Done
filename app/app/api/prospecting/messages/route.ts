...
import { isPublicSourceUrl } from '@/lib/prospecting';
...
export async function POST(request: Request) {
  ...
  const { data: profile } = await supabase.from('growth_profiles').select('sender_email, emergency_stop').eq('id', prospect.profile_id).single();
