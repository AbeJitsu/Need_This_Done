...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!z.string().uuid().safeParse((await params).id).success) return NextResponse.json({ error: 'Invalid message.' }, { status: 400 });
  ...
  const { data, error } = await supabase.rpc('record_outreach_decision', { target_message_id: (await params).id, target_decision: parsed.data.decision, target_subject: parsed.data.subject || '', target_body: parsed.data.body || '', target_next_action_at: parsed.data.nextActionAt || null });
