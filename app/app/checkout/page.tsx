...
export default function CheckoutPage() {
  ...
  const proceedToPayment = async () => {
    ...
    try {
      ...
      const chargeAmount = payInFull ? cartTotal : calculateDeposit(cartTotal);
                ...
                <StripeElementsWrapper clientSecret={clientSecret}>
                  <PaymentForm
                    ...
                    submitText={payInFull
                      ? `Pay in Full $${((cart?.total || 0) / 100).toFixed(2)}`
                      : `Pay Deposit $${(calculateDeposit(cart?.total || 0) / 100).toFixed(2)}`
                    }
                  />
