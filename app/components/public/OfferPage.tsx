...
export default function OfferPage({ eyebrow, title, introduction, commitment, included, excluded, cta, href }: OfferPageProps) {
  return <main id="main-content" className="bg-[#f7f4ed] text-[#183229]">
    <section className="border-b border-[#183229]/10 bg-[#183229] text-white"><div className="mx-auto max-w-6xl px-5 py-16 sm:px-8 md:py-24"><p className="text-xs font-bold uppercase tracking-[.2em] text-[#b9d5bd]">{eyebrow} · A focused starting point</p><h1 className="mt-5 max-w-4xl font-playfair text-5xl font-black leading-[.98] sm:text-6xl md:text-7xl">{title}</h1><p className="mt-7 max-w-2xl text-lg leading-8 text-[#dce8dd] md:text-xl">{introduction}</p><Link href={href} className="mt-9 inline-flex min-h-12 items-center gap-2 rounded-full bg-[#e4eee6] px-7 py-3 font-bold text-[#183229] hover:bg-white">{cta}<ArrowRight className="h-4 w-4" aria-hidden="true" /></Link></div></section>
