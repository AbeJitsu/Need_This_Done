...
export interface ProspectDossier { id: string; profile_id: string; prospect_id: string; task_id: string | null; company_name: string; official_website_url: string; icp_reason: string; observed_evidence: Array<{ claim: string; citationUrls: string[] }>; citations: Array<{ url: string; title: string; excerpt: string }>; recommended_offer_angle: string; contact_path: { type: string; value: string; email?: string }; suggested_subject: string; suggested_body: string; model_id: string; review_status: 'pending_review' | 'promoted' | 'rejected'; promoted_message_id: string | null; created_at: string; }
export interface ModelUsageLedgerEntry { id: string; task_id: string | null; usage_kind: 'benchmark' | 'research'; model_id: string; reserved_cost: number; actual_cost: number | null; status: 'reserved' | 'reconciled' | 'released' | 'overage'; local_usage_date: string; created_at: string; }
